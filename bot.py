...
# How long after alert to send updates (seconds)
UPDATE_TIMES = [600, 1800, 3600]  # 10 min, 30 min, 1 hour
# ============================================================

alerted = set()
# tracked[pair_addr] = {ticker, name, entry_mcap, entry_price, token_addr, alerted_at, message_id, updates_sent}
tracked = {}
...
def send_telegram(message, reply_to=None):
    """Send message, optionally as a reply. Returns message_id."""
...
def fetch_current_data(pair_addr):
    """Get current price and MC for a tracked pair."""
    try:
        r = requests.get(
...
def send_gain_update(pair_addr, minutes_elapsed):
    """Fetch current price and reply to original alert with update."""
    if pair_addr not in tracked:
        return

    t = tracked[pair_addr]
    current = fetch_current_data(pair_addr)

    if not current or current["mcap"] <= 0:
        print(f"[UPDATE] Could not fetch data for {t['ticker']}")
        return
    ...
    if entry_mcap > 0:
        pct_change = ((current_mcap - entry_mcap) / entry_mcap) * 100
    ...
    if multiplier >= 5:       perf_emoji = "🔥🔥🔥 MASSIVE RUNNER"
    elif multiplier >= 3:     perf_emoji = "🚀🚀 3X+ HIT"
    elif multiplier >= 2:     perf_emoji = "📈📈 2X HIT"
    elif multiplier >= 1.5:   perf_emoji = "📈 +50% GAIN"
    elif multiplier >= 1.1:   perf_emoji = "🟡 Slight gain"
    elif multiplier >= 0.8:   perf_emoji = "🔴 Down from entry"
    else:                     perf_emoji = "💀 Dumped hard"
    ...
    print(f"[UPDATE] Sent {minutes_elapsed}min update for {t['ticker']} — {multiplier:.2f}x")


# ── UPDATE SCHEDULER ─────────────────────────────────────────
def schedule_updates(pair_addr):
    """Schedule gain updates in background threads."""
    for delay in UPDATE_TIMES:
        ...
        t = threading.Thread(target=send_update, daemon=True)
        t.start()
...
def get_rugcheck(token_address):
    ...
    try:
        ...
        for risk in risks:
            n = risk.get("name",  "").lower()
            l = risk.get("level", "").lower()
...
def check_socials(pair):
    ...
    for s in info.get("socials", []):
        t   = s.get("type","").lower()
        url = s.get("url", "")
        if "twitter" in t or "x.com" in url.lower(): twitter       = url
        if "telegram" in t:                           telegram_link = url
...
def scan_and_alert():
    ...
    for c in candidates[:5]:
        ...
        # Send and get message_id for replies
        message_id = send_telegram(msg)

        alerted.add(pair_addr)

        # Track for gain updates
        if message_id:
            tracked[pair_addr] = {
                "ticker":       ticker,
                "name":         name,
        ...
        sent += 1
        print(f"[ALERT] {ticker} score {score} msg_id {message_id}")
        time.sleep(1)
